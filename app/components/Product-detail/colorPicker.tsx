...
const ColorPicker = ({ product }: any) => {
  ...
  const isUniqueColor = !product.colors || product.colors.length === 0;
    ...
    <>
      {isUniqueColor ? (
        <div className="flex text-center my-5">
          ...
          <h5 className="text-[14px] font-normal">&nbsp;UNIQUE COLOR</h5>
        </div>
      ) : (
        <div className="flex text-center my-5">
          <h4 className="text-[14px] font-bold mr-5">Color</h4>
      ...
      <div className='flex'>
        {!isUniqueColor && product.colors.map((item: any, i: number) => (
          ...
          hover:border-[#3c52a2] cursor-pointer" key={i}>
            <div className={colorpiker === item.color ? "colorPiked" : "colorBox"}
              ...
              onClick={() => {
                dispatch(selectedColor(item.name));
